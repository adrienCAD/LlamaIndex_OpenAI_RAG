...
# Load the data and create the VectorStoreIndex
def load_data(llm_engine):
    with st.spinner(text="Loading and indexing the Streamlit docs – hang tight! This should take one minute."):
        ...
        if os.path.exists("./chroma_db"):
            ...
            # Create VectorStoreIndex using reconstructed docs
            index = VectorStoreIndex.from_vector_store(vector_store, storage_context=storage_context)

            return index

        else:
            # Create and initialize Vector store if it doesn't exist
            reader = SimpleDirectoryReader(input_dir="../PDFs", recursive=True)
            docs = reader.load_data()

            # OpenAI Settings
            # Initialize the OpenAI Embedding and SentenceSplitter
            ...
            # Initialize Chroma DB client and Vector store
            db = chromadb.PersistentClient(path="./chroma_db")
            chroma_collection = db.get_or_create_collection("quickstart")
            vector_store = ChromaVectorStore(chroma_collection=chroma_collection)
            storage_context = StorageContext.from_defaults(vector_store=vector_store)
...
index = load_data(llm_engine)  # Assign both index and docs from load_data()
